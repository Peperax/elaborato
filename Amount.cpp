...
Amount::Amount(const double v): cents(static_cast<long long>(std::round(std::abs(v) * 100))) {
    if (!isValid()) {
...
void Amount::setValue(const double v) {
    const long long oldCents = cents;
...
std::string Amount::toString() const {
    ...
    std::string s = std::to_string(absoluteCents / 100) + ".";
